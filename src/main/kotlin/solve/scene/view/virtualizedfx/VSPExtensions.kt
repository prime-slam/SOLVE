...
fun <T, C : GridCell<T>?> wrapGridWithVsp(
    ...
    vsp.contentBoundsProperty().bind(
        ObjectBindingBuilder.build<VirtualBounds>().setMapper {
    ...
    val vSource =
        MappingSource.of<Position, Number>(grid.positionProperty()).setTargetUpdater(
            Mapper.of { `val`: Position ->
                val helper = grid.gridHelper
                val max = helper.maxVScroll()
                if (max != 0.0) `val`.y / max else 0
            }
        ) { _: Number?, n: Number -> vsp.vVal = n.toDouble() }.setSourceUpdater(
            Mapper.of { `val`: Number ->
                val helper = grid.gridHelper
    ...
    val hSource =
        ...
        ) { _: Number?, n: Number -> vsp.hVal = n.toDouble() }.setSourceUpdater(
            Mapper.of { `val`: Number ->
                val helper = grid.gridHelper
                val currPos = grid.position
...
private class VirtualScrollPaneWithOutOfFramesLayerSkin<T, C : GridCell<T>?>(
    ...
    init {
        outOfFramesLayer.layoutXProperty().bind(
            ObjectBindingBuilder.build<Number>().setMapper {
        ...
        outOfFramesLayer.layoutYProperty().bind(
            ObjectBindingBuilder.build<Number>().setMapper {
