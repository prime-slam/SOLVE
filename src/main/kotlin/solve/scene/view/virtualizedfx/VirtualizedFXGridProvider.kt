...
object VirtualizedFXGridProvider : GridProvider {
    ...
    private fun <T, C : GridCell<T>?> wrapGridWithVsp(grid: VirtualGrid<T, C>): VirtualScrollPane {
        val vsp = VirtualScrollPaneWithOutOfFramesLayer(grid)
        val bindings = MFXBindings.instance()
        vsp.orientation = Orientation.VERTICAL
        vsp.contentBoundsProperty().bind(ObjectBindingBuilder.build<VirtualBounds>()
            .setMapper {
        ...
        val vSource = MappingSource.of<Position, Number>(grid.positionProperty())
            .setTargetUpdater(Mapper.of { `val`: Position ->
                val helper = grid.gridHelper
                val max = helper.maxVScroll()
                if (max != 0.0) `val`.y / max else 0
            }) { o: Number?, n: Number -> vsp.vVal = n.toDouble() }
            .setSourceUpdater(Mapper.of { `val`: Number ->
                val helper = grid.gridHelper
        ...
        bindings.bindBidirectional(vsp.vValProperty())
            ...
            .get()
        val hSource = MappingSource.of<Position, Number>(grid.positionProperty())
            .setTargetUpdater(Mapper.of { `val`: Position ->
                val helper = grid.gridHelper
                val max = helper.maxHScroll()
                if (max != 0.0) `val`.x / max else 0
            }) { o: Number?, n: Number -> vsp.hVal = n.toDouble() }
            .setSourceUpdater(Mapper.of { `val`: Number ->
                val helper = grid.gridHelper
                val currPos = grid.position
    ...
    private class VirtualScrollPaneWithOutOfFramesLayer<T, C : GridCell<T>?>(private val virtualGrid: VirtualGrid<T, C>) :
        VirtualScrollPane(virtualGrid) {
        override fun createDefaultSkin(): Skin<*> {
            return VirtualScrollPaneWithOutOfFramesLayerSkin(this, virtualGrid)
        }
    }
    ...
    private class VirtualScrollPaneWithOutOfFramesLayerSkin<T, C : GridCell<T>?>(
        ...
        init {
            ...
            outOfFramesLayer.layoutXProperty().bind(ObjectBindingBuilder.build<Number>()
                .setMapper {
                    -vsp.hVal * virtualGrid.gridHelper.maxHScroll() // This approach is used by original VSP for VirtualGrid
                }
                .addSources(vsp.hValProperty())
                .get()
            )
            outOfFramesLayer.layoutYProperty().bind(ObjectBindingBuilder.build<Number>()
                .setMapper {
                    -vsp.vVal * virtualGrid.gridHelper.maxVScroll()
                }
                .addSources(vsp.vValProperty())
                .get()
            )
