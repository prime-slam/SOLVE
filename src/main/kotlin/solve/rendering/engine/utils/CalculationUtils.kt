...
fun pointToSegmentDistanceSquared(
    ...
    var t = (
        (pointPosition.x - segmentStartPosition.x) * (segmentEndPosition.x - segmentStartPosition.x) +
            (pointPosition.y - segmentStartPosition.y) * (segmentEndPosition.y - segmentStartPosition.y)
        ) / segmentLength
    t = max(0f, min(1f, t))
    val a = Vector2f(
        segmentStartPosition.x + t * (segmentEndPosition.x - segmentStartPosition.x),
